...
def test_self_gate_requires_named_legislation():
    ...
    assert _query_target("Explain the Companies Act 2006") == ("Companies Act", "2006", "/ukpga/2006/46")


async def test_fetches_known_act_section_as_structured_source():
    def handler(request: httpx.Request) -> httpx.Response:
        assert request.url.path == "/ukpga/2006/46/section/386/data.xml"
        return httpx.Response(200, text="""<Legislation><Metadata><Title>Companies Act 2006, section 386</Title></Metadata><Body><P1>Every company must keep adequate accounting records.</P1></Body></Legislation>""")
...
async def test_unknown_named_act_uses_official_atom_search():
    feed = """<feed xmlns="http://www.w3.org/2005/Atom"><entry><title>Some Business Act 2024</title><link href="https://www.legislation.gov.uk/ukpga/2024/99" /></entry></feed>"""
