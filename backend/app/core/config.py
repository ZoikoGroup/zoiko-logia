...
class Settings(BaseSettings):
    ...
    # Opt-in workaround for DNS64/NAT64 resolvers (some VPNs, tethering,
    # certain ISPs) that answer with synthesized 64:ff9b::/96 addresses a
    # host with no NAT64 gateway cannot reach — every connection then hangs
    # until it times out instead of using the IPv4 address that works. See
    # app/core/net_ipv4.py for the full mechanism.
    #
    # Defaults off: on a genuinely IPv6-only host behind a *working* NAT64,
    # preferring IPv4 would break connectivity that currently succeeds.
    # Enable only on hosts that have real IPv4 connectivity.
    DB_PREFER_IPV4: bool = False
