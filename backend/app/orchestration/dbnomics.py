...
_STOPWORDS = {
    ...
    # Chart-type and comparison words. DBnomics full-text search ANDs its
    # terms, so one presentation word that appears in no series name zeroes
    # the whole result set: "compare gdp india line" returned 0 datasets
    # where "gdp india" returned 3. "chart"/"graph" were already here; the
    # words that actually name the chart were not.
    "line", "bar", "pie", "donut", "doughnut", "scatter", "plot", "area",
    "trend", "table", "visualize", "visualise", "display", "compare",
    "comparison", "versus", "against", "between", "across", "using",
}
