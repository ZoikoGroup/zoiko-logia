...
import { getEscalations, actOnEscalation, type Escalation } from "@/lib/safety-api";
...
function slaStatus(deadline: string | null): { 
  ...
  // Assume a default 24h window for scaling the progress bar
...
export default function EscalationQueuePage() {
  ...
  const overdue = escalations.filter((e) => {
    ...
    return new Date(e.sla_deadline).getTime() < now() && e.status !== "RESOLVED" && e.status !== "REFUSED";
  });
  ...
  return (
    <main className="flex-1 overflow-y-auto p-6 pt-0 space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: "Total Cases", value: escalations.length, tone: "info" as const },
          { label: "Pending Review", value: pending.length, tone: "warn" as const },
          { label: "Over SLA", value: overdue.length, tone: "bad" as const },
          { label: "Resolved", value: escalations.filter((e) => e.status === "RESOLVED").length, tone: "ok" as const },
        ].map((m) => (
          <div 
            key={m.label}
      ...
      {errorMsg && (
        <div className="rounded-xl border-2 border-bad/30 bg-bad/5 p-4 text-xs text-bad leading-relaxed flex items-start gap-2 animate-shake shadow-lg">
          ...
          <div>
            <span className="font-bold uppercase tracking-wider">Maker-Checker Exception:</span> {errorMsg}
          </div>
        </div>
      )}
